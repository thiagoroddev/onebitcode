...
function addSpaceship(name: string, pilot: string, crewLimit: number) {
  const spaceship = {
    ...
    crewLimit,     // Limite máximo de tripulantes
    crew: [],      // Array de tripulantes (inicia vazio)
    inMission: false // Status de missão (inicia como false)
  }

  spaceships.push(spaceship) // Adiciona a nova nave ao array
...
function findSpaceship(name: string) {
  ...
  spaceship = spaceships.find((ship) => {
    return ship.name === name // Procura nave com nome correspondente
  });
  
  return spaceship // Retorna a nave encontrada ou undefined
}
